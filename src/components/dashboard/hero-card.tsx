 ...
 *
 * Dismissible banner: closing it collapses to a measured pixel height via a
 * plain CSS transition (not framer-motion — it can't tween to/from "auto"
 * reliably) so content below slides up instead of just vanishing.
 */
export function HeroCard() {
  ...
  return (
    ...
    >
      {/* Buffer so the card's drop shadow has room to render before the
          overflow-hidden collapse wrapper above clips it. */}
      <div ref={contentRef} className="p-3 sm:p-4">
        ...
        >
          ...
          <button
            ...
            className="absolute right-4 top-4 z-20 grid size-8 place-items-center rounded-full bg-black/15 text-white transition-colors hover:bg-black/25"
          >
            <X className="size-4" />
          </button>
